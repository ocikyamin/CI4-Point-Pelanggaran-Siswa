<!-- Modal -->
<div class="modal fade" id="modal-edit-pelanggaran"data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form method="post" id="form-edit-pelanggaran">
        ...
        <input type="hidden" name="id" value="<?=$data_pelanggaran['id']?>">
      ...
      <div class="modal-body">
        ...
        <div class="row">
          ...
          <div class="form-floating mb-3">
              ...
              <option value="<?=$p['id']?>" <?=$p['id']==$pelanggaran_id['jenis_id'] ? 'selected' : null?>><?=$p['jenis']?></option>
              <?php endforeach; ?>
              </select>
              ...
              <option value="<?=$data_pelanggaran['pelanggaran_id']?>" selected> <?=$pelanggaran_id['nama_pelanggaran']?></option>
              o
            </select>
            ...
            <div class="form-floating mb-3">
            <textarea class="form-control" name="keterangan" placeholder="Catatan Tambahan / Keterangan" id="keterangan" style="height: 100px"><?=$data_pelanggaran['keterangan']?></textarea>
            <label for="keterangan">Tindak Lanjut / Keterangan / Catatan Tambahan</label>
            </div>

            <div class="form-floating mb-3">
            <input type="date" name="tgl_kejadian" class="form-control" id="tgl_kejadian" value="<?=$data_pelanggaran['tgl_kejadian']?>">
...
<script>
  ...
  $('#form-edit-pelanggaran').submit(function (e) { 
    e.preventDefault();
    $.ajax({
      type: "post",
      url: "<?=base_url('teacher/student/pelanggaran/update')?>",
      data: $(this).serialize(),
      dataType: "json",
      success: function (response) {
       // If in-valid 
       if (response.error) {
        ...
        // Is Sukses 
        if (response.sukses) {
            $('#modal-edit-pelanggaran').modal('hide')
            Toastify({
            text: 'Data Pelanggaran diperbarui'     
            }).showToast();

           
          window.location= '<?=base_url('teacher/history')?>';
        }
