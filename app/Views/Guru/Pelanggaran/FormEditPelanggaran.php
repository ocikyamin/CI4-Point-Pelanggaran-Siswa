  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header shadow-sm">
        <h1 class="modal-title fs-5">Edit Rekam Pelanggaran</h1>
        <button type="button" class="btn-close text-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form method="post" id="form-entry-pelanggaran" enctype="multipart/form-data">
        <?=csrf_field()?>
        <input type="hidden" name="id" value="<?=$d['id']?>">
        <input type="hidden" name="periode_langgar_id" value="<?=$d['periode_langgar_id']?>">
        <input type="hidden" name="semester_langgar_id" value="<?=$d['semester_langgar_id']?>">
        <input type="hidden" name="user_created_id" value="<?=$d['user_created_id']?>">
      ...
      <div class="modal-body">
        ...
        <div class="row">
          ...
          <div class="form-floating mb-3">
              ...
              <option value="<?=$p['id']?>" <?=$p['id']==$d['jenis_id'] ? 'selected':null ?>><?=$p['jenis']?></option>
              <?php endforeach; ?>
              </select>
              ...
              <div class="form-floating mb-3">
              <select class="form-select" name="pelanggaran_id" id="item-pelanggaran">
              <option value="<?=$d['pelanggaran_id']?>" selected> <?=$d['nama_pelanggaran']?> </option>
              <option value=""> Daftar Pelanggaran</option>
            </select>
              <label for="item-pelanggaran">Pilih Nama Pelanggaran</label>
              </div>
            <div class="form-floating mb-3">
            <input type="date" name="tgl_kejadian" class="form-control" id="tgl_kejadian" value="<?=$d['tgl_kejadian']?>">
...
<div class="form-check form-check-inline">
    <input class="form-check-input" type="radio" name="status_tindak_lanjut" id="tl-selesai" value="selesai" <?=$d['status_tindak_lanjut']=='selesai'? 'checked':null ?> >
    <label class="form-check-label" for="tl-selesai">Selesai</label>
</div>
<div class="form-check form-check-inline">
    <input class="form-check-input" type="radio" name="status_tindak_lanjut" id="tl-teruskan" value="diteruskan" <?=$d['status_tindak_lanjut']=='diteruskan'? 'checked':null ?> >
...
<!-- Jika Tindak Lanjut selesai -->
 <div class="mb-3 mt-3" id="selesai-section" style="display: none;">
    <div class="form-floating mb-3">
    <input type="date" name="tgl_penyelesaian" class="form-control" id="tgl_penyelesaian" value="<?=$d['tgl_penyelesaian']?>">
    <label for="tgl_penyelesaian">Tanggal Peyelesaian </label>
    </div>
<div class="form-floating" >
    <textarea class="form-control" name="keterangan" placeholder="Catatan Tambahan / Keterangan" id="keterangan" style="height: 100px"><?=$d['keterangan']?></textarea>
    ...
    <select class="form-select" name="teruskan_ke" id="teruskan-ke">
      ...
      if (Jabatan()) {
        foreach (Jabatan() as $j) {
          ?>
<option value="<?=$j['id']?>" <?=$d['teruskan_ke']==$j['id'] ? 'selected':null ?>><?=$j['jabatan']?></option>
